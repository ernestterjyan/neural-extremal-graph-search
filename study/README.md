...
The supplementary command checks and reuses completed compatible cells. A fresh full rerun should use a separate source checkout without the extracted completed artifacts. The source bundle captures research snapshot `690efc9`, before the subsequent archive-packaging commit. The evidence archive, source bundle and outer handoff manifest are now committed together. The bundle does not recursively contain itself; its recorded source revision identifies the complete research code and reports.
